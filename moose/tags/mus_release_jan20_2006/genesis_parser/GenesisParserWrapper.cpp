...
map< string, string >& sliSrcLookup()
{
	...
	src[ "SUBSTRATE n" ] = "";
	src[ "SUBSTRATE n vol" ] = ""; // For concchans.
	src[ "PRODUCT n vol" ] = ""; // For concchans.
...
map< string, string >& sliDestLookup()
{
	...
	dest[ "SUBSTRATE n vol" ] = "efflux"; // For channels.
	dest[ "PRODUCT n vol" ] = "influx";
...
void do_loadtab( int argc, const char** argv, Shell* s )
{
	s->commandFuncLocal( argc, argv );
}
