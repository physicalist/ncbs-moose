...
class SquidDemo(QtGui.QMainWindow):
    def __init__(self, *args):
        ...
        self.squidThread = sqthr.SquidThread('/squid', self)
        self.resize(600,400)
        ...
        self.connect(self.inputPanel.inputs["blockNa"], QtCore.SIGNAL("stateChanged(int)"), self.toggleNaChannels)
        self.connect(self.inputPanel.inputs["blockK"], QtCore.SIGNAL("stateChanged(int)"), self.toggleKChannels)
        self.connect(self.squidThread, QtCore.SIGNAL("output()"), self.updatePlot)
    ...
    def slotRun(self):
        ...
        self.emit(QtCore.SIGNAL("doRun(double, double, double, double, double, double)"), totalTime, preTime, pulseWidth, updateInterval, injectBase, injectHigh)
        
    def slotReset(self):
        print 'Resetting the simulation'
        self.requestQueue.put([sqthr.SquidThread.DO_RESET])
    ...
    def updatePlot(self):
        print "qtSuid - updating plot"
        vm = self.squidThread.squid.getVm()
        print "qtSuid - data received"
        t = self.data.xEnd 
        self.data.xEnd = t + len(vm)*SIMDT
        x = numpy.arange(t, self.data.xEnd, SIMDIT)
        y = numpy.array(vm)
    ...
    def toggleNaChannels(self):
        self.requestQueue.put([sqthr.SquidThread.TOGGLE_NA])

    def toggleKChannels(self):
        self.requestQueue.put([sqthr.SquidThread.TOGGLE_K])

    def doExit(self):
        self.requestQueue.put([sqthr.SquidThread.DO_QUIT])
        self.close()
